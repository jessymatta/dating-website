...
const api_tokennn = JSON.parse(localStorage.getItem('token'));
const favorites_url = "http://127.0.0.1:8000/api/v0.1/get_favorites";
...
async function getFavorites() {
    const response = await axios.get(
        ...
        {
            headers: {
                'Authorization': `bearer ${api_tokennn}`
            }
        }
    )

    const profiles = response.data.fav_users;
